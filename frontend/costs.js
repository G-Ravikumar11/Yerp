...
function renderCostStats() {
    ...
    host.innerHTML =
        ...
        statCard('Committed cost', formatCurrency(s.cost || 0)) +
        statCard('Margin', formatCurrency(s.margin || 0)) +
        statCard('Billed', formatCurrency(s.billed || 0)) +
        statCard('Unpaid', formatCurrency(s.unpaid || 0)) +
...
function renderCosts() {
    ...
    body.innerHTML = COSTS.rows.map(function (r) {
        ...
        return '<tr>' +
            ...
            '<td class="text-right">' + formatCurrency(r.sold) + '</td>' +
            '<td class="text-right">' + formatCurrency(r.cost) +
                (r.over_budget > 0
                    ? '<div style="font-size:0.72rem;color:var(--danger-color);">' +
                      formatCurrency(r.over_budget) + ' over budget</div>' : '') + '</td>' +
            ...
            '<td class="text-right">' + formatCurrency(r.billed) + '</td>' +
